classdef InternalModesOperator
    ...
    methods
        function self = InternalModesOperator(options)
            % Create an empty physical-coordinate operator.
            %
            % - Topic: Create operators
            % - Declaration: op = InternalModesOperator(options)
        ...
        function M = matrix(self, solver)
            ...
            if self.form ~= "strong"
                error("InternalModesOperator:UnsupportedForm", ...
            ...
            for iTerm = 1:length(self.terms)
                coefficient = InternalModesOperator.evaluateCoefficient(self.terms(iTerm).coefficient, z, context);
    ...
    methods (Static)
        function op = strong()
            ...
            %
            % - Topic: Create operators
            % - Declaration: op = InternalModesOperator.strong()
            % - Returns op: empty strong-form operator
            op = InternalModesOperator(form="strong");
        end
        ...
        function values = evaluateCoefficient(coefficient, z, context)
            ...
            % - Topic: Assemble operators
            % - Developer: true
            % - Declaration: values = InternalModesOperator.evaluateCoefficient(coefficient,z,context)
