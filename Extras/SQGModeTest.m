...
% 
% im = InternalModesConstantStratification(N0=N0,rho0=rho_0,zIn=zIn,zOut=z,latitude=lat);
% 
k = 10.^linspace(log10(1e-5),log10(1e-1),10)';
% psi_t = im.SurfaceModesAtWavenumber( k );
% psi_b = im.BottomModesAtWavenumber( k );
% 
% im_spec = InternalModesFiniteDifference(rho=rho,zIn=zIn,zOut=z,latitude=lat);
% psi_t_spec = im_spec.SurfaceModesAtWavenumber( k );
% psi_b_spec = im_spec.BottomModesAtWavenumber( k );
% 
% im_wkb = InternalModesWKB(rho=rho,zIn=zIn,zOut=z,latitude=lat);
...
im_spec = InternalModesSpectral(rho=rho,zIn=zIn,zOut=z,latitude=lat);
im_wkb = InternalModesWKB(rho=rho,zIn=zIn,zOut=z,latitude=lat);
