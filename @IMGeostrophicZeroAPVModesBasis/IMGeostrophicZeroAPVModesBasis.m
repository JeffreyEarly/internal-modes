    ...
    methods (Access = private)
        ...
        function pages = expandMatrixPages(self,matrix,name)
            ...
            if size(matrix,1) ~= nEndpoints || size(matrix,2) ~= nEndpoints
                error("IMGeostrophicZeroAPVModesBasis:InvalidPencilShape", "%s must have nEndpoints rows and columns.",name);
            end
            if size(matrix,3) == 1
            ...
            elseif size(matrix,3) == nK
                ...
                error("IMGeostrophicZeroAPVModesBasis:InvalidPencilShape", "%s must have one page or nK pages.",name);
            end
            for iK = 1:nK
                scale = max(1,norm(pages(:,:,iK),"fro"));
                if norm(pages(:,:,iK)-pages(:,:,iK).',"fro") > 1e-12*scale
                    error("IMGeostrophicZeroAPVModesBasis:NonSymmetricPencil", "%s must be symmetric on every wavenumber page.",name);
                end
                pages(:,:,iK) = self.symmetrize(pages(:,:,iK));
            end
        end

        function [vectors,eigenvalues,rightSignatures,residuals] = solveSymmetricPencil(self,leftMatrix,rightMatrix,ordering)
            leftMatrix = self.expandMatrixPages(leftMatrix,"leftMatrix");
            rightMatrix = self.expandMatrixPages(rightMatrix,"rightMatrix");
            nEndpoints = numel(self.endpoints);
            nK = numel(self.k);
            ...
            for iK = 1:nK
                ...
                if any(~isfinite(lambda)) || any(abs(imag(lambda)) > 1e-10*imaginaryScale)
                    error("IMGeostrophicZeroAPVModesBasis:InvalidPencilSpectrum", "The matrix pencil must have a complete finite real spectrum on every wavenumber page.");
                end
                C = real(C);
                lambda = real(lambda);
                for iMode = 1:nEndpoints
                    ...
                    if abs(rightNorm) <= 1e-12*normScale
                        error("IMGeostrophicZeroAPVModesBasis:NullRightMetric", "A pencil eigenvector has a numerically zero right-metric norm.");
                    end
    ...
    methods (Static, Access = private)
        ...
        function pages = symmetrizePages(pages)
            for iPage = 1:size(pages,3)
                pages(:,:,iPage) = IMGeostrophicZeroAPVModesBasis.symmetrize(pages(:,:,iPage));
            end
        end
    end
end
