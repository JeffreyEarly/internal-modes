...
if ~spec.hasInnerProduct
    recipe = IMProjectionRecipe(available=false,reason=string(spec.reason),variable=options.variable,columnLabels=string(self.modeNumber),normalization=self.normalizationName(self.normalization),provenance=provenance,zDomain=self.zDomain);
    return;
end
