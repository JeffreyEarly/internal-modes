...
started = tic;
[p,recipe] = self.projection(z,weights,page=options.page,variable=options.variable,columns=options.columns);
...
assessment = IMBasisAssessment(p,identity=identity,columnKind=metadata.columnKind,prefixCounts=options.prefixCounts,leakage=options.leakage,products=options.products,coverage=options.coverage,constructionSeconds=constructionSeconds);
end
