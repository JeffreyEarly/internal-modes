...
| [`Analytical bases`](./analytical-bases) | you want exact constant- or exponential-stratification internal modes or geostrophic zero-APV boundary modes |
| [`Inner products and norms`](./inner-products-and-norms) | you need to choose between the signed projection pairing and the positive majorant used for error magnitudes |
...
- [`IMMeanDensityAnomalyModes`](./core/immeandensityanomalymodes) defines the signed generalized-energy displacement EVP. Its [`IMMeanDensityAnomalyModesBasis`](./core/immeandensityanomalymodesbasis) solves `G` and obtains the aligned mean-pressure `F` modes by surface-referenced spectral integration.
...
- [`IMAnalyticalGeostrophicZeroAPVModesBasis`](./analytical-bases/imanalyticalgeostrophiczeroapvmodesbasis) stores exact canonical or rotated zero-APV modes with the same endpoint-response, quadratic-form, and matrix-pencil contracts as the numerical basis.
- [`Inner products and norms`](./inner-products-and-norms) explains why modal projection uses the signed Pontryagin pairing while error magnitudes use its induced positive Hilbert majorant.
- [`IMBasisSet`](./core/imbasisset) constructs scalar point-limited rules and fixed-point fits. [`IMInternalModesBasis`](./core/iminternalmodesbasis) specializes those builders for aligned `F/G` families: [`IMInternalModesDiscreteTransform`](./discrete-transforms/iminternalmodesdiscretetransform) stores the shared physical snapshot and variable-qualified matrices, [`IMInternalModesDiscreteTransformAssessment`](./discrete-transforms/iminternalmodesdiscretetransformassessment) records worst-channel retained-prefix policies, and [`IMInternalModesQuadratureWeightFit`](./discrete-transforms/iminternalmodesquadratureweightfit) preserves the stacked shared-weight fit. Mean-density-anomaly transforms project `G` while retaining aligned diagnostic `F` synthesis. [`IMGeostrophicTransform`](./discrete-transforms/imgeostrophictransform) composes a generalized-energy APV transform with boundary-normalized or rotated zero-APV coordinates at positive horizontal wavenumber. The corresponding `IMDiscreteTransform`, `IMDiscreteTransformAssessment`, and `IMQuadratureWeightFit` classes remain the scalar API.
